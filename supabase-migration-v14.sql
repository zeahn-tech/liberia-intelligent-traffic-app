...
-- Run each section separately in a new SQL Editor tab
-- =====================================================
...
-- =====================================================
-- SECTION 2: GLOBAL SEARCH FUNCTION
-- =====================================================
...
DECLARE
  ...
  v_tsquery TSQUERY;
BEGIN
  v_search_term := '%' || COALESCE(p_query, '') || '%';
  ...
  WITH search_results AS (
    SELECT * FROM (
      ...
      SELECT
        ...
        CASE
          WHEN i.vehicle_plate ILIKE v_search_term THEN 4
          WHEN i.title ILIKE v_search_term THEN 3
          WHEN i.id::text ILIKE v_search_term THEN 3
          WHEN i.description ILIKE v_search_term THEN 2
          WHEN i.location_address ILIKE v_search_term THEN 2
          ELSE 1
        END AS relevance
      ...
      WHERE (p_query IS NULL OR p_query = '' OR
        ...
        AND (p_type_filter IS NULL OR p_type_filter IN ('incident', 'all'))
      ...
      WHERE (p_query IS NULL OR p_query = '' OR
        ...
        AND (p_type_filter IS NULL OR p_type_filter IN ('evidence', 'all'))
      ...
      SELECT
        ...
        CASE WHEN a.officer_verified THEN 'verified'::text ELSE 'pending'::text END AS status,
        NULL::text,
      ...
      WHERE (p_query IS NULL OR p_query = '' OR
        ...
        AND (p_type_filter IS NULL OR p_type_filter IN ('anpr', 'all'))
      ...
      WHERE (p_query IS NULL OR p_query = '' OR
        ...
        AND (p_type_filter IS NULL OR p_type_filter IN ('citizen_report', 'all'))
      ...
      WHERE (p_query IS NULL OR p_query = '' OR
        ...
        AND (p_type_filter IS NULL OR p_type_filter IN ('person', 'all'))
    ) all_results
  ),
  counted AS (
  ...
  SELECT jsonb_build_object(
    ...
    'total', (SELECT total FROM counted LIMIT 1)
  )
  INTO v_results
  FROM sorted s;
...
-- =====================================================
-- SECTION 3: SAVE SEARCH HISTORY FUNCTION
-- =====================================================
...
-- =====================================================
-- SECTION 4: GET RECENT SEARCHES FUNCTION
-- =====================================================
