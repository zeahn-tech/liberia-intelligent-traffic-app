...
--
-- Run order: 00001_init.sql (this file) → 00002_functions.sql → 00003_camera_entities.sql
...
DO $$ BEGIN
  CREATE TYPE public.user_role AS ENUM (
    ...
    'investigator', 'evidence_officer', 'system_auditor', 'citizen'
  );
...
DO $$ BEGIN
  CREATE TYPE public.violation_severity AS ENUM ('minor', 'moderate', 'serious', 'critical');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE public.evidence_type AS ENUM ('photo', 'video', 'document', 'audio', 'other');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE TYPE public.ai_provider AS ENUM ('vly', 'gemini', 'openai', 'custom');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
  CREATE TYPE public.stolen_status AS ENUM ('active', 'recovered', 'closed');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
-- ─── 2.1 PROFILES — User profiles with role info ────────────
CREATE TABLE IF NOT EXISTS public.profiles (
  id                  UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  email               TEXT NOT NULL UNIQUE,
  full_name           TEXT NOT NULL DEFAULT '',
  role                public.user_role NOT NULL DEFAULT 'traffic_officer',
...
-- ─── 2.2 ROLES — Role definitions with hierarchy ──────────
...
CREATE TABLE IF NOT EXISTS public.violation_types (
  ...
  severity        public.violation_severity NOT NULL DEFAULT 'moderate',
  is_active       BOOLEAN NOT NULL DEFAULT true,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.vehicle_owners (
  ...
  owner_type      TEXT NOT NULL DEFAULT 'individual' CHECK (owner_type IN ('individual', 'company', 'government', 'other')),
  id_type         TEXT,
  id_number       TEXT,
  phone           TEXT,
  email           TEXT,
...
-- ─── 2.8 INCIDENT LOGS — Audit history for incidents ─────
...
-- ─── 2.9 INCIDENT VIOLATIONS — Junction table ────────────
CREATE TABLE IF NOT EXISTS public.incident_violations (
  ...
  severity          public.violation_severity,
  description       TEXT,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ─── 2.11 INVOLVED PERSONS ──────────────────────────────
CREATE TABLE IF NOT EXISTS public.involved_persons (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  incident_id   UUID NOT NULL REFERENCES public.incidents(id) ON DELETE CASCADE,
  full_name     TEXT NOT NULL,
  id_type       TEXT NOT NULL DEFAULT 'other' CHECK (id_type IN ('drivers_license', 'national_id', 'passport', 'other')),
  id_number     TEXT,
  address       TEXT,
  phone         TEXT,
  email         TEXT,
...
CREATE TABLE IF NOT EXISTS public.evidence (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  incident_id           UUID NOT NULL REFERENCES public.incidents(id) ON DELETE CASCADE,
  type                  public.evidence_type NOT NULL DEFAULT 'photo',
  file_url              TEXT,
  file_path             TEXT,
  description           TEXT,
  file_size             BIGINT,
  ...
  sha256_hash           TEXT,
  officer_notes         TEXT,
  evidence_status       TEXT NOT NULL DEFAULT 'original' CHECK (evidence_status IN ('original', 'processed', 'reviewed', 'archived', 'expunged')),
  original_file_url     TEXT,
  original_file_hash    TEXT,
  source                TEXT NOT NULL DEFAULT 'officer_upload',
...
-- ─── 2.14 EVIDENCE CUSTODY — Chain of custody ────────────
CREATE TABLE IF NOT EXISTS public.evidence_custody (
  ...
  performed_by    UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  from_officer    UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  to_officer      UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  ip_address      TEXT,
  user_agent      TEXT,
  details         JSONB DEFAULT '{}',
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
...
-- ─── 2.15 EVIDENCE VERSIONS — Immutable versions ─────────
...
-- ─── 2.16 STORAGE FILES — Supabase Storage tracking ──────
...
-- ─── 2.17 AI ANALYSES — AI analysis results ─────────────
CREATE TABLE IF NOT EXISTS public.ai_analyses (
  id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  incident_id             UUID NOT NULL REFERENCES public.incidents(id) ON DELETE CASCADE,
  evidence_id             UUID REFERENCES public.evidence(id) ON DELETE SET NULL,
  provider_id             public.ai_provider NOT NULL DEFAULT 'vly',
  status                  public.analysis_status NOT NULL DEFAULT 'pending',
  error_message           TEXT,
  violation_type          TEXT NOT NULL DEFAULT 'unknown',
  confidence_score        REAL NOT NULL DEFAULT 0.0 CHECK (confidence_score >= 0 AND confidence_score <= 1.0),
  ...
  vehicle_color           TEXT,
  license_plate           TEXT,
  license_plate_confidence REAL CHECK (license_plate_confidence IS NULL OR (license_plate_confidence >= 0 AND license_plate_confidence <= 1.0)),
  detected_objects        JSONB DEFAULT '[]',
  violations              JSONB DEFAULT '[]',
  ai_summary              TEXT,
  severity                TEXT,
  processing_time_ms      INTEGER,
  recommended_review      BOOLEAN NOT NULL DEFAULT false,
...
CREATE TABLE IF NOT EXISTS public.ai_analysis_jobs (
  ...
  status          public.analysis_status NOT NULL DEFAULT 'pending',
  error_message   TEXT,
  result_id       UUID REFERENCES public.ai_analyses(id) ON DELETE SET NULL,
...
-- ─── 2.19 ANPR SCANS — License plate recognition ──────
CREATE TABLE IF NOT EXISTS public.anpr_scans (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  incident_id           UUID NOT NULL REFERENCES public.incidents(id) ON DELETE CASCADE,
  plate_text            TEXT NOT NULL,
  normalized_plate      TEXT NOT NULL DEFAULT '',
  plate_confidence      REAL NOT NULL DEFAULT 0.0 CHECK (plate_confidence >= 0 AND plate_confidence <= 1.0),
...
CREATE TABLE IF NOT EXISTS public.stolen_vehicles (
  ...
  status          public.stolen_status NOT NULL DEFAULT 'active',
...
CREATE TABLE IF NOT EXISTS public.citizen_reports (
  ...
  report_type         TEXT NOT NULL CHECK (report_type IN (
                        'traffic_violation', 'accident', 'road_hazard', 'police_assistance', 'general_complaint', 'other'
                      )),
  violation_type      TEXT,
  ...
  status              TEXT NOT NULL DEFAULT 'submitted' CHECK (status IN (
                        'submitted', 'under_review', 'accepted', 'rejected', 'converted_to_case', 'closed'
                      )),
  status_notes        TEXT,
  reviewed_by         UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  reviewed_at         TIMESTAMPTZ,
...
-- ─── 2.22 OFFICER NOTIFICATIONS ─────────────────────────
CREATE TABLE IF NOT EXISTS public.officer_notifications (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id         UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  type            TEXT NOT NULL CHECK (type IN (
                    'serious_violation', 'wanted_vehicle', 'stolen_vehicle', 'major_accident',
                    'road_closure', 'incident_escalation', 'assignment', 'investigation_update',
                    'evidence_update', 'ai_analysis_complete', 'system_alert', 'message'
                  )),
  title           TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.notification_preferences (
  ...
  notification_type   TEXT NOT NULL CHECK (notification_type IN (
                        'serious_violation', 'wanted_vehicle', 'stolen_vehicle', 'major_accident',
                        'road_closure', 'incident_escalation', 'assignment', 'investigation_update',
                        'evidence_update', 'ai_analysis_complete', 'system_alert'
                      )),
  channel_in_app      BOOLEAN NOT NULL DEFAULT true,
  channel_push        BOOLEAN NOT NULL DEFAULT false,
  channel_email       BOOLEAN NOT NULL DEFAULT false,
  channel_sms         BOOLEAN NOT NULL DEFAULT false,
  min_priority        TEXT NOT NULL DEFAULT 'normal' CHECK (min_priority IN ('low', 'normal', 'high', 'urgent')),
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.report_history (
  ...
  report_type     TEXT NOT NULL CHECK (report_type IN ('case_summary', 'violation_report', 'evidence_report', 'investigation_report', 'analytics_export', 'custom')),
...
-- ─── 2.28 SYNC QUEUE — Offline sync coordination ──────
...
-- ─── 3.1 COUNTIES — All 15 Liberia counties ─────────────
...
-- ─── 3.2 DISTRICTS — Liberia districts ─────────────────
CREATE TABLE IF NOT EXISTS public.districts (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  county_code   TEXT NOT NULL REFERENCES public.counties(code) ON DELETE CASCADE,
  name          TEXT NOT NULL,
  center_lat    DECIMAL(10,7),
  center_lng    DECIMAL(10,7),
  is_active     BOOLEAN NOT NULL DEFAULT true,
...
CREATE TABLE IF NOT EXISTS public.prediction_results (
  ...
  title             TEXT NOT NULL,
  description       TEXT,
  confidence        REAL NOT NULL DEFAULT 0.0 CHECK (confidence >= 0 AND confidence <= 100),
  ...
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
  -- Disclaimer: All predictions are estimates, not facts
  CONSTRAINT prediction_disclaimer CHECK (true)
);
...
-- ─── 4.4 HIGH RISK ROADS ────────────────────────────
CREATE TABLE IF NOT EXISTS public.high_risk_roads (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- ============================================================
-- 5. TRAFFIC CAMERA INFRASTRUCTURE TABLES
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.traffic_cameras (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name              TEXT NOT NULL,
  installation_type TEXT NOT NULL DEFAULT 'fixed' CHECK (installation_type IN (
                      'fixed', 'mobile', 'cctv', 'highway', 'speed', 'red_light',
                      'anpr', 'panoramic', 'drone', 'body', 'dashcam', 'other'
                    )),
  stream_url        TEXT,
  stream_type       TEXT DEFAULT 'hls' CHECK (stream_type IN ('rtsp', 'hls', 'webrtc', 'mjpeg', 'usb', 'file', 'unknown')),
  latitude          DECIMAL(10,7) NOT NULL DEFAULT 0,
  longitude         DECIMAL(10,7) NOT NULL DEFAULT 0,
  location_address  TEXT,
  county_id         TEXT,
  status            TEXT NOT NULL DEFAULT 'disconnected' CHECK (status IN (
                      'disconnected', 'connecting', 'connected', 'streaming', 'reconnecting', 'error', 'offline'
                    )),
  is_active         BOOLEAN NOT NULL DEFAULT true,
  manufacturer      TEXT,
  model             TEXT,
...
CREATE TABLE IF NOT EXISTS public.camera_events (
  ...
  event_type        TEXT NOT NULL CHECK (event_type IN (
                      'vehicle_detected', 'license_plate_captured', 'speed_violation',
                      'red_light_violation', 'illegal_turn', 'pedestrian_detected',
                      'obstacle_detected', 'accident_detected', 'congestion_detected',
                      'camera_offline', 'camera_online', 'maintenance_alert'
                    )),
  event_data        JSONB DEFAULT '{}',
  media_url         TEXT,
  detected_plate    TEXT,
  detected_speed    REAL,
  confidence        REAL CHECK (confidence IS NULL OR (confidence >= 0 AND confidence <= 1.0)),
  location_lat      DECIMAL(10,7),
  location_lng      DECIMAL(10,7),
...
-- ─── 6.2 OFFICER ASSIGNMENTS ─────────────────────────
CREATE TABLE IF NOT EXISTS public.officer_assignments (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  officer_id      UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  assignment_type TEXT NOT NULL CHECK (assignment_type IN ('patrol', 'checkpoint', 'investigation', 'special_operation', 'administrative')),
  location_lat    DECIMAL(10,7),
  location_lng    DECIMAL(10,7),
  location_address TEXT,
  county_code     TEXT,
...
CREATE TABLE IF NOT EXISTS public.patrol_units (
  ...
  unit_type       TEXT NOT NULL DEFAULT 'vehicle' CHECK (unit_type IN ('vehicle', 'motorcycle', 'foot', 'bicycle', 'water', 'air')),
  vehicle_plate   TEXT,
  officer_ids     UUID[] DEFAULT '{}',
  current_lat     DECIMAL(10,7),
  current_lng     DECIMAL(10,7),
  status          TEXT NOT NULL DEFAULT 'available' CHECK (status IN ('available', 'on_patrol', 'responding', 'at_scene', 'off_duty', 'maintenance')),
  county_code     TEXT,
  is_active       BOOLEAN NOT NULL DEFAULT true,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE TABLE IF NOT EXISTS public.road_conditions (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  condition_type  TEXT NOT NULL CHECK (condition_type IN ('construction', 'accident', 'flooding', 'road_closure', 'hazard', 'congestion', 'event', 'other')),
  ...
  reported_by_type TEXT DEFAULT 'officer' CHECK (reported_by_type IN ('officer', 'citizen', 'system', 'camera')),
...
CREATE POLICY "Users can view their own profile"
  ...
  USING (auth.uid() = id OR public.get_current_user_role() IN ('system_administrator', 'national_commissioner', 'regional_commander', 'traffic_commander', 'police_supervisor', 'system_auditor'));
...
-- ─── 7.2 VIOLATION TYPES RLS ─────────────────────────
CREATE POLICY "Authenticated users can read violation types"
  ON public.violation_types FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "Admins and commanders can manage violation types"
  ON public.violation_types FOR ALL
  USING (public.get_current_user_role() IN ('system_administrator', 'traffic_commander'));

-- ─── 7.3 VEHICLES RLS ────────────────────────────────
CREATE POLICY "Authorized roles can read vehicles"
  ON public.vehicles FOR SELECT
  USING (public.get_current_user_role() IN (
    'system_administrator', 'national_commissioner', 'regional_commander', 'traffic_commander',
    'police_supervisor', 'traffic_officer', 'investigator', 'evidence_officer', 'system_auditor'
  ));

CREATE POLICY "Officers and investigators can manage vehicles"
  ON public.vehicles FOR ALL
  USING (public.get_current_user_role() IN ('system_administrator', 'traffic_officer', 'police_supervisor', 'investigator'));

-- ─── 7.4 INCIDENTS RLS ───────────────────────────────
CREATE POLICY "Officers can read own incidents, supervisors read all"
  ON public.incidents FOR SELECT
  USING (
    auth.uid() = officer_id
    OR auth.uid() = assigned_to
    OR public.get_current_user_role() IN ('system_administrator', 'national_commissioner', 'regional_commander', 'traffic_commander', 'police_supervisor', 'investigator', 'system_auditor')
  );
...
CREATE POLICY "Officers can update own incidents"
  ON public.incidents FOR UPDATE
  USING (
    auth.uid() = officer_id
    OR public.get_current_user_role() IN ('system_administrator', 'police_supervisor', 'investigator')
  );
...
CREATE POLICY "Officers and investigators can create evidence"
  ON public.evidence FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Evidence officers and investigators can update"
  ON public.evidence FOR UPDATE
  USING (public.get_current_user_role() IN ('system_administrator', 'evidence_officer', 'investigator', 'traffic_officer'));

CREATE POLICY "Only admins can delete evidence"
  ON public.evidence FOR DELETE
  USING (public.get_current_user_role() = 'system_administrator');

-- ─── 7.6 CITIZEN REPORTS RLS ─────────────────────────
...
CREATE POLICY "Authorized police can update citizen reports"
  ON public.citizen_reports FOR UPDATE
  USING (public.get_current_user_role() IN ('system_administrator', 'police_supervisor', 'investigator', 'traffic_officer'));

-- ─── 7.7 NOTIFICATIONS RLS ───────────────────────────
...
CREATE POLICY "Auditors and admins can read audit logs"
  ...
  USING (public.get_current_user_role() IN ('system_administrator', 'system_auditor'));

CREATE POLICY "System can insert audit logs"
  ON public.audit_logs FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

-- ─── 7.9 SYSTEM SETTINGS RLS ─────────────────────────
CREATE POLICY "Anyone can read public settings"
  ON public.system_settings FOR SELECT
  USING (is_public = true OR public.get_current_user_role() IN ('system_administrator', 'national_commissioner', 'traffic_commander'));
...
-- ─── 7.10 GEOGRAPHY RLS ──────────────────────────────
CREATE POLICY "Authenticated users can read geography data"
  ON public.counties FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can read districts"
  ON public.districts FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can read police regions"
  ON public.police_regions FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can read police stations"
  ON public.police_stations FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can read roads"
  ON public.roads FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can read checkpoints"
  ON public.checkpoints FOR SELECT USING (auth.role() = 'authenticated');

-- ─── 7.11 CAMERA INFRASTRUCTURE RLS ─────────────────
CREATE POLICY "Authorized roles can read traffic cameras"
  ON public.traffic_cameras FOR SELECT
  USING (auth.role() = 'authenticated' AND public.get_current_user_role() IN (
    'system_administrator', 'national_commissioner', 'regional_commander', 'traffic_commander',
    'police_supervisor', 'traffic_officer', 'investigator', 'system_auditor'
  ));

CREATE POLICY "Admins and commanders can manage cameras"
...
-- ─── 7.12 PREDICTIVE ANALYTICS RLS ──────────────────
CREATE POLICY "Authorized roles can read predictions"
  ON public.prediction_models FOR SELECT
  USING (public.get_current_user_role() IN ('system_administrator', 'regional_commander', 'traffic_commander', 'police_supervisor', 'investigator') OR auth.uid() IS NOT NULL);

CREATE POLICY "Authorized roles can read prediction results"
...
-- ─── 8.2 updated_at trigger function ─────────────────
...
-- ─── 8.3 Auto-create profile on user signup ──────────
...
-- ─── 8.4 Auto-create default notification preferences ─
